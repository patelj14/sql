...
SELECT c.customer_last_name, c.customer_first_name, ROUND(SUM(cp.quantity * cp.cost_to_customer_per_qty)) AS Spent
FROM customer c
...
SELECT customer_id, strftime('%Y-%m', market_date) AS date, SUM(quantity * cost_to_customer_per_qty) AS spent
FROM customer_purchases
